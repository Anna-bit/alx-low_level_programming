 ...
 *
 * Description: Print exit code 98 if file not elf.
 */
void check_elf(unsigned char *e_ident)
{
	int turtle;

	for (turtle = 0; turtle < 4; turtle++)
	{
		if (e_ident[turtle] != 127 &&
		    e_ident[turtle] != 'E' &&
		    e_ident[turtle] != 'L' &&
		    e_ident[turtle] != 'F')
		{
 ...
 * @e_ident: pointer to array having the magic numbers.
 *
...
void print_magic(unsigned char *e_ident)
{
	...
	for (tortoise = 0; tortoise < EI_NIDENT; tortoise++)
	{
		printf("%02x", e_ident[tortoise]);

		if (tortoise == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
...
/**
 * print_data - Function that prints the data of Elf header.
...
/**
 * print_osabi - Function that prints the os/abi.
 * @e_ident: pointer to an array.
 */
void print_osabi(unsigned char *e_ident)
{
 ...
 * @e_ident: pointer to array with the version.
 */
void print_abi(unsigned char *e_ident)
{
...
/**
 * main - Displays information contained in the file
 * @argc: number of arguments.
 * @argv: arguments vector.
 *
 * Return: Always 0 success.
 *
 * Description: Print exit code 98 if function fails
 */
